...
router.get('/atividades/:tipo', function (req, res) {

    if(req.query.filtro=='maislonga'){
        Record.AtividadeEspecificaMaisLonga(req.params.tipo)
        .then(dados => res.jsonp(dados)) // jsonp é mais seguro contra ataques (hackers)
        .catch(erro => res.status(500).send("Erro na listagem: " + erro))
    }
    else{
    Record.listarTipo(req.params.tipo)
        .then(dados => res.jsonp(dados)) // jsonp é mais seguro contra ataques (hackers)
        .catch(erro => res.status(500).send("Erro na listagem: " + erro))
    }
});
