    ...
    <v-flex>
      <v-card>
        <v-card-text class="text-xs-center">
          <h1 v-if="this.label"> Editora: {{this.label}} </h1>
...
<script>
  ...
  export default {
    ...
    mounted: async function (){
      ...
      try{
        var response = await axios.get(lhost+'/query/publisher_info_id/'+this.id);
        this.publisher_info = response.data.results.bindings
        // console.log(this.animeResponse) // debug
      }
      catch(e){
        return(e);
      }
      this.publisher_info.forEach(item => {
        // console.log(item)
